...
git clone https://github.com/BigBirdReturns/GhostBox
git clone https://github.com/BigBirdReturns/axm-core   # for the Spectra query proof
...
pip install -e ./GhostBox      # the exit lives here now
pip install -e ./axm-core --no-deps   # Spectra, for `axm ask` / the query proof
```
...
The sample lives at `GhostBox/samples/nhs_synthetic_exit/`. One command seals
and self-verifies it:

```bash
axm-exit GhostBox/samples/nhs_synthetic_exit --out ./nhs_exit_out
```
